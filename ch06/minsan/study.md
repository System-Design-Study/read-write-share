...
👉🏻 _이 내용 또한 지금 단계에서는 지엽적으로 느껴져서 기록은 생략함. 관련 내용은 `교재 108쪽`을 참고_
...
3. 메모리 캐시가 가득 차거나 임계치에 도달하면, 데이터는 **디스크**에 있는 **SSTable**에 기록된다. _(SSTable - 키, 값 순서쌍을 정렬된 리스트 형태로 관리하는 테이블)_
